...
function ppoRenderChart(entries) {
  ...
  const bars = sorted.map((e, i) => {
    ...
    const barColor = oee >= 85 ? '#34d399' : oee >= 60 ? '#fbbf24' : '#f87272';
    const label = e.date.slice(5);
    return `
      <rect x="${x}" y="${y}" width="${w}" height="${height - padB - y}" fill="${barColor}" rx="2" />
      <text x="${x + w / 2}" y="${height - padB + 14}" text-anchor="middle" font-size="9" fill="#7d8ba8">${label}</text>
      <text x="${x + w / 2}" y="${y - 4}" text-anchor="middle" font-size="9" fill="#e9eef7">${oee.toFixed(0)}%</text>
    `;
  }).join('');
  ...
  chartEl.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" style="width:100%;height:auto;background:var(--bg-alt);border-radius:10px;">
      <line x1="${padL}" y1="${benchmarkY}" x2="${width - padR}" y2="${benchmarkY}" stroke="#4d97ff" stroke-dasharray="4,4" stroke-width="1.2" />
      <text x="${width - padR}" y="${benchmarkY - 4}" text-anchor="end" font-size="10" fill="#4d97ff">85% world-class</text>
      ${bars}
    </svg>
  `;
}
