...
function pspRenderChart(points, fit) {
  ...
  const circles = points.map(p =>
    `<circle cx="${xFor(p.hours)}" cy="${yFor(p.score)}" r="4" fill="#34d399" />`
  ).join('');
  ...
  chartEl.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" style="width:100%;height:auto;background:var(--bg-alt);border-radius:10px;">
      <line x1="${xFor(lineX1)}" y1="${yFor(lineY1)}" x2="${xFor(lineX2)}" y2="${yFor(lineY2)}" stroke="#4d97ff" stroke-width="1.5" />
      ${circles}
    </svg>
  `;
}
...
function pspRenderFit(fit) {
  ...
  let warning = '';
  if (fit.r2 < 0.5) {
    warning = '<br><span style="color:#f87272;">Fit is weak — other factors likely matter more than hours studied for you.</span>';
  }
  fitEl.innerHTML =
